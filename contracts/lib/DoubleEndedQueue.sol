...
library DoubleEndedQueue {
  ...
  function begin(Uint256Deque storage deque) internal view returns (uint256) {
    return uint256(deque._begin);
  }

  function end(Uint256Deque storage deque) internal view returns (uint256) {
    return uint256(deque._end);
  }

  function set(Uint256Deque storage deque, uint256 index, uint256 value) internal {
    require(index <= type(uint128).max, "Index out of bounds");
    unchecked {
      deque._data[uint128(index)] = value;
    }
  }

  /**
   * @dev 判断给定的值是否存在于队列中，并返回布尔值和对应的索引。
   * @param deque 队列
   * @param value 要查找的值
   * @return found 是否找到
   * @return index 找到的索引，未找到返回 0
   */
  function isIn(
    Uint256Deque storage deque,
    uint256 value
  ) internal view returns (bool found, uint256 index) {
    uint128 currentIndex = deque._begin;
    uint128 dequeLength = uint128(length(deque));
    uint128 i = 0;
    while (i < dequeLength) {
      if (deque._data[currentIndex] == value) {
        return (true, uint256(currentIndex));
      }
      unchecked {
        currentIndex++;
        i++;
